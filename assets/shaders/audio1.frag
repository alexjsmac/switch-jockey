...
vec2 map(vec3 p)
{
  ...
  vec4 noise = TEXTURE2D(iChannel1, floor(q.zz / 1.0));

  float freq =  atan(p.x, -p.y) / 3.1415 * 2.0 + fract(q.z) * 3.1415 * 2.0;

  float audio = TEXTURE2D(iChannel0, vec2((freq), 0.0)).r;
  float audio2 = TEXTURE2D(iChannel0, vec2(fract(0.01*q.z), 0.0)).r;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec2 noise = TEXTURE2D(iChannel0, _p).rg;
